...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiClient.post('password_reset/', { email: email.trim() });
      setSubmitted(true);
    } catch (err) {
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
        ...
        <div className="flex flex-col items-center mb-6 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Weka email yako hapa chini ili tukutumie code/link ya kubadilisha password.
          </p>
        </div>
          ...
          <div className="text-center space-y-4">
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-emerald-400 flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-slate-300">
                Tumetuma barua pepe kwenda <span className="font-bold text-white">{email}</span>. Tafadhali fungua inbox yako.
              </p>
            </div>
            
            <Link 
              to="/reset-password" 
              className="w-full py-3.5 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold rounded-2xl flex items-center justify-center transition text-sm"
            >
            ...
            <Link 
              to="/login" 
              className="block text-xs font-semibold text-slate-400 hover:text-white transition pt-2"
            >
              Rudi Kwenye Login
            </Link>
          </div>
